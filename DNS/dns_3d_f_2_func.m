...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [UVW,Ek,U_hat] = dns_3d_f_2_func(paras,c1,c2,d,e,U_hat,sample_interval)
tic;
L = paras{1};
N = paras{3};
nu = 1/paras{4};
...
UVW = zeros(N,N,N,1+LEN);
for ind=1:dim
    UVW(:,:,:,1) = UVW(:,:,:,1) + (real(ifftn(U_hat{ind}))).^2;
end
UVW(:,:,:,1) = sqrt(UVW(:,:,:,1));
Ek = zeros(LEN+1,1);
p_DFT = 1/(N^3);
...
for ind = 1:Ninteration
    ...
    if mod(ind,interval)==0
        jnd = ind/interval;
        for knd=1:dim
            UVW(:,:,:,jnd+1) = UVW(:,:,:,jnd+1) + (real(ifftn(U_hat{knd}))).^2;
        end
        UVW(:,:,:,jnd+1) = sqrt(UVW(:,:,:,jnd+1));
