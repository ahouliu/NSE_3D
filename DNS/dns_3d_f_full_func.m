...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [U_data,Ek,U_hat] = dns_3d_f_full_func(paras,c1,c2,d,e,U_hat,sample_interval)
tic;
L = paras{1};
N = paras{3};
nu = 1/paras{4};
...
LEN = round(Ninteration/interval);
U_data = {zeros(N,N,N,1+LEN),zeros(N,N,N,1+LEN), zeros(N,N,N,1+LEN)};
for ind=1:dim
    U_data{ind}(:,:,:,1) = real(ifftn(U_hat{ind}));
end
Ek = zeros(LEN+1,1);
p_DFT = 1/(N^3);
...
for ind = 1:Ninteration
    ...
    if mod(ind,interval)==0
        jnd = ind/interval;
        U_data{1}(:,:,:,1+jnd) = real(ifftn(U_hat{1}));
        U_data{2}(:,:,:,1+jnd) = real(ifftn(U_hat{2}));
        U_data{3}(:,:,:,1+jnd) = real(ifftn(U_hat{3}));
        Ek(1+jnd) = 0.5*L^2*N2*p_DFT;
    end
